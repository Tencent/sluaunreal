...
#define default_state(L) ((pb_State*)default_lstate(L))
    ...
    static lua_Integer pb_posrelat(lua_Integer pos, size_t len) {
        if (pos >= 0) return pos;
    ...
    static lua_Integer rangerelat(lua_State *L, int idx, lua_Integer r[2], size_t len) {
        r[0] = pb_posrelat(luaL_optinteger(L, idx, 1), len);
        r[1] = pb_posrelat(luaL_optinteger(L, idx + 1, len), len);
        if (r[0] < 1) r[0] = 1;
    ...
    static uint64_t lpb_tointegerx(lua_State *L, int idx, int *isint) {
        ...
        const char *s, *os;
#if LUA_VERSION_NUM >= 503
        int64 v = (int64)lua_tointegerx(L, idx, isint);
        if (*isint) return (uint64_t)v;
#else
...
# define setmode(a,b)  ((void)0)
    ...
    static int Lio_read(lua_State *L) {
        ...
        lua_pushlstring(L, (char*)content.GetData(), content.Num());
        return 1;
    }
...
#define LPB_INITSTACKLEN 2
    ...
    static int lpb_unpackloc(lua_State *L, int *pidx, int top, int fmt, lpb_SliceEx *s, int *prets) {
        ...
        case '*': case '+':
            argcheck(L, *pidx <= top, 1, "format argument exceed");
            if (fmt == '*')
                li = pb_posrelat(luaL_checkinteger(L, *pidx++), len);
            else
    ...
    static int Lslice_level(lua_State *L) {
        ...
        if (!lua_isnoneornil(L, 2)) {
            lpb_SliceEx *se;
            lua_Integer level = pb_posrelat(luaL_checkinteger(L, 2), s->used);
    ...
    static int Lslice_leave(lua_State *L) {
        lpb_Slice *s = check_lslice(L, 1);
        lua_Integer count = pb_posrelat(luaL_optinteger(L, 2, 1), s->used);
